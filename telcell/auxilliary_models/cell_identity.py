...
RADIO_MAPPING = {RADIO_GSM: "2G", RADIO_UMTS: "3G", RADIO_LTE: "4G",
                 RADIO_NR: "5G", RADIO_EGSM: "2G"}
...
class CellIdentity:
    ...
    @staticmethod
    def create(*, radio: Optional[Radio] = None, mcc: Optional[int] = None,
               mnc: Optional[int] = None, lac: Optional[int] = None,
               ci: Optional[int] = None,
               eci: Optional[int] = None, ) -> "CellIdentity":
        ...
        if radio == Radio.GSM.value:
            ...
            raise ValueError(
                "either `ci` or `eci` should be provided, but not both")
    ...
    def parse(cell_identity: str) -> "CellIdentity":
        ...
        * NR/MCC-MNC-CI
        ...
        groups = {key: convert_value(key, value) for key, value in
                  m.groupdict().items()}
...
class CellGlobalIdentity(CellIdentity):
    ...
    def __eq__(self, other):
        return (isinstance(other, CellGlobalIdentity) and super().__eq__(
            other) and self.lac == other.lac and self.ci == other.ci)
...
class EutranCellGlobalIdentity(CellIdentity):
    ...
    def __eq__(self, other):
        return (isinstance(other, EutranCellGlobalIdentity) and super().__eq__(
            other) and self.eci == other.eci)

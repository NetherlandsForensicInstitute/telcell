...
def parse_measurements_csv(path: Union[str, Path]) -> List[Track]:
    ...
    with open(path, "r") as f:
        ...
        measurements_sorted = sorted(
            reader, key=lambda row: (row["owner"], row["device"], row["timestamp"])
        )

        for (owner, device), group in groupby(
            measurements_sorted, key=lambda row: (row["owner"], row["device"])
        ):
            ...
            measurements = [
                Measurement(
                    coords=geopy.Point(
                        latitude=float(row["cellinfo.wgs84.lat"]),
                        longitude=float(row["cellinfo.wgs84.lon"]),
                    ),
    ...
    with open(path, "r") as f:
        ...
        for row in reader:
            ...
            measurement = Measurement(
                coords=geopy.Point(
                    latitude=float(row["antenna_lat"]),
                    longitude=float(row["antenna_lon"]),
                ),
                timestamp=None,
                extra={
            ...
            if key not in data:
                data[key] = {
                    "location": Measurement(
                        coords=geopy.Point(
                            latitude=float(row["lm_lat"]),
                            longitude=float(row["lm_lon"]),
                        ),
                        timestamp=None,
                        extra={},
                    ),

...
from telcell.utils.transform import (
    get_switches,
    create_track_pairs,
    is_colocated,
    MeasurementPair,
    categorize_measurement_by_coordinates,
    _sort_pairs_based_on_rarest_location,
...
def test_create_track_pairs(test_data):
    ...
    assert (
        len([pair for pair in pairs if is_colocated(*pair)]) == 4
    )  # same source: 1 A and 3 B
    assert len(pairs) == 4 + 11


def test_sort_by_time_diff_for_same_location_rarity(max_delay):
    measurement_tmp = partial(
        Measurement, geopy.Point(latitude=1, longitude=1), extra={}
    )
    ...
    switches_one_location = [
        MeasurementPair(
            measurement_tmp(timestamp=t_0),
            measurement_tmp(timestamp=t_0 + timedelta(seconds=seed.randint(1, 30))),
        )
        for _ in range(10)
    ]
    background_b = Track("", "", [s.measurement_b for s in switches_one_location])
    sorted_pairs = _sort_pairs_based_on_rarest_location(
        switches_one_location,
        background_b,
    ...
    assert all(
        x[1].measurement_b.timestamp <= y[1].measurement_b.timestamp
...
def test_sort_by_location_rarity_for_same_time_diff(max_delay):
    ...
    switches_one_time_diff = [
        MeasurementPair(
            measurement_tmp(coords=geopy.Point(latitude=1, longitude=1)),
            measurement_tmp(
                coords=geopy.Point(latitude=2, longitude=seed.randint(1, 4))
            ),
        )
        for _ in range(10)
    ]
    background_b = Track("", "", [s.measurement_b for s in switches_one_time_diff])
    sorted_pairs = _sort_pairs_based_on_rarest_location(
        switches_one_time_diff,
        background_b,
        categorize_measurement_for_rarity=categorize_measurement_by_coordinates,
    ...
    for (correct_count, correct_lon_b), (count, pair) in zip(
        correct_counts_lon_b, sorted_pairs
    ):
    ...
    background_a = Track("", "", [s.measurement_a for s in switches_one_time_diff])
    sorted_pairs = _sort_pairs_based_on_rarest_location(
        switches_one_time_diff,
        background_a,
        categorize_measurement_for_rarity=categorize_measurement_by_coordinates,
...
def test_sort_outside_bin(max_delay):
    ...
    switches = [
        MeasurementPair(
            measurement_tmp(timestamp=t_0),
            measurement_tmp(timestamp=t_0 + timedelta(minutes=seed.randint(-4, 4))),
        )
        for _ in range(10)
    ]
    ...
    assert (
        len(sorted_pairs) == 5
    )  # some pairs with too large time difference are filtered
...
def test_round_lat_lons(max_delay):
    ...
    switches = [
        MeasurementPair(
            measurement_tmp(coords=geopy.Point(latitude=1, longitude=1)),
            measurement_tmp(coords=geopy.Point(latitude=1, longitude=1 + (i / 10000))),
        )
        for i in range(10)
    ]
    background_b = Track("", "", [s.measurement_b for s in switches])

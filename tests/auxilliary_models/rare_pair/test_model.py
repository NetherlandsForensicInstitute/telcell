...
from telcell.auxilliary_models.rare_pair.coverage_model import (
    AngleDistanceClassificationCoverageModel,
)
...
def test_angle_distance_coverage_model():
    ...
    diameter = 2000
    resolution = 50
    classification_model = AngleDistanceClassificationCoverageModel(
        resolution, diameter, clf
    )
    classification_model._probabilities = np.ones([180, diameter // DISTANCE_STEP])

    predictor = Predictor({(4, (0, 0)): classification_model})
    assert (
        pytest.approx(predictor.get_probability_e_h(test_measurement, test_measurement))
        == 1.0
    )

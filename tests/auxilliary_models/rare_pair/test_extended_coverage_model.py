...
from telcell.auxilliary_models.rare_pair.coverage_model import (
    ExtendedAngleDistanceClassificationCoverageModel,
)
from telcell.auxilliary_models.rare_pair.utils import DISTANCE_STEP
from telcell.data.models import Measurement, point_to_rd


@pytest.fixture
def dummy_model() -> ExtendedAngleDistanceClassificationCoverageModel:
    coverage_model = ExtendedAngleDistanceClassificationCoverageModel(
        1000, 100, 11000, 1000, DecisionTreeClassifier()
    )
    coverage_model._probabilities = np.ones(
        [181, coverage_model.diameter // DISTANCE_STEP]
    )
...
def test_measurement(dummy_model) -> Measurement:
    point = geopy.Point(latitude=52.0449566305567, longitude=4.3585472613577965)
    x, y = point_to_rd(point)
    # Move the point so it fits exactly in the middle of the grid, nice for the angle and distance checks
    gridpoint = (
        GridPoint(x, y)
        .stick_to_resolution(dummy_model.resolution)
        .move(dummy_model.resolution / 2, dummy_model.resolution / 2)
    )
    return Measurement(
        gridpoint.as_point(),
...
def test_get_angles_and_distances(dummy_model, test_measurement):
    angles, distances = dummy_model._extract_features(
        test_measurement, dummy_model.measurement_locations(test_measurement)
    )

    angles = angles.numpy().reshape(
        -1, int(dummy_model.diameter / dummy_model.resolution)
    )
    distances = distances.numpy().reshape(
        -1, int(dummy_model.diameter / dummy_model.resolution)
    )
...
def test_get_probabilities(dummy_model, test_measurement):
    ...
    assert probabilities.outer.values[0, 0] == 0.0
    assert probabilities.outer.values[4, 4] == 1.0

...
from telcell.auxilliary_models.geography import (
    GridPoint,
    manhattan_distance,
    Grid,
    EmptyGrid,
...
def test_stick_to_resolution():
    ...
    assert GridPoint(100000, 100000) == GridPoint(90000, 110000).stick_to_resolution(
        100000
    )
...
def test_grid():
    ...
    assert np.sum(_grid.values == 1.0) == 96
...
def test_edges():
    ...
    """
    _grid = EmptyGrid(
        100, 10, GridPoint(1000, 1000), (GridPoint(1040, 1040), GridPoint(1060, 1060))
    )
...
def test_coords():
    ...
    _grid = EmptyGrid(
        100,
        10,
        GridPoint(1000, 10000),
        (GridPoint(1040, 10040), GridPoint(1060, 10060)),
    )
    assert _grid.x_coords == [
        1005,
        1015,
        1025,
        1035,
...
def test_meshgrid_coords():
    ...
    """
    _grid = EmptyGrid(
        500, 10, GridPoint(1000, 1000), (GridPoint(1200, 1200), GridPoint(1300, 1300))
    )
...
def test_move_values():
    ...
    """
    p = GridPoint(10000, 500000)
    _norm_grid = Grid(500, 10, p, np.ones((50, 50))).normalize(1)
    ...
    assert pytest.approx(_norm_grid.sum()) == 0.5
    ...
    assert pytest.approx(_norm_grid.sum()) == 0.25

    _norm_grid = Grid(500, 10, p, np.ones((50, 50))).normalize(1)
    _norm_grid = _norm_grid.move(_norm_grid.southwest.move(100, 300))
    assert pytest.approx(_norm_grid.sum()) == 0.32
